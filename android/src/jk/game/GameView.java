...
class GameView extends GLSurfaceView {
	...
	private void init(boolean translucent, int depth, int stencil) {
		...
		mRenderer = new Renderer(mContext, mGameActivity);
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (e.getAction() == MotionEvent.ACTION_DOWN) {
			j_JNILib.touchPressed((int) x, (int) y);
		} else if (e.getAction() == MotionEvent.ACTION_UP) {
			j_JNILib.touchReleased((int) x, (int) y);
		}
		return true;
	}
	...
	private static class Renderer implements GLSurfaceView.Renderer {
		...
		public void onSurfaceChanged(GL10 gl, int width, int height) {
			Log.i(TAG, "jk.game.GameView.Renderer.onSurfaceChanged");
			if (!isResume){
		...
		public void onPause() {
			j_JNILib.pause();
			Log.i(TAG, "GameView.Renderer.onPause");
		}
